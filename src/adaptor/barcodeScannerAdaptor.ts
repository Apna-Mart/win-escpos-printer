...
export class BarcodeScannerAdapter implements ReadableDevice {
  ...
  constructor(
    ...
    assert(terminalDevice.meta.deviceType === 'scanner', 'Terminal device is not a barcode scanner')
    const path = terminalDevice.path;
    const baudRate = terminalDevice.meta.baudrate;
    assert(baudRate !== 'not-supported', 'Barcode scanner does not support baudrate change')
    this.device = new SerialPort({ path, baudRate, endOnClose: true, autoOpen: false });
  }
