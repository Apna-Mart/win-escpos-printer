...
export class WeightScaleAdapter implements ReadableDevice {
  ...
  constructor(
    ...
    assert(terminalDevice.meta.deviceType === 'scale', 'Terminal device is not a weight scale')
    const path = terminalDevice.path;
    const baudRate = terminalDevice.meta.baudrate;
    assert(baudRate !== 'not-supported', 'Weight scale does not support baudrate change')
