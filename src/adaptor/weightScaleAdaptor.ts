...
export class WeightScaleAdapter implements ReadableDevice {
	...
	constructor(
		...
		logger.info('WeightScaleAdapter created successfully', {
			deviceId: terminalDevice.id,
			path,
			baudRate,
			keepAliveIntervalMs,
		...
		return withExponentialBackoff(async () => {
			return new Promise<void>((resolve, reject) => {
				...
				this._device.open((err) => {
					...
					this._isOpen = true;
					logger.info('Weight scale connection established successfully', {
						deviceId: this.terminalDevice.id,
						path: this.terminalDevice.path,
						callbackCount: this.dataCallbacks.size,
					});
	...
	async close() {
		...
		return new Promise<void>((resolve, _reject) => {
			...
			this._device.close(() => {
				this._isOpen = false;
				logger.info('Weight scale connection closed successfully', {
					deviceId: this.terminalDevice.id,
					path: this.terminalDevice.path,
